...
Ensuring equitable model performance across diverse market segments is a critical requirement for this pipeline. Our bias analysis evaluates feature drift (Population Stability Index, PSI) and model-level fairness across multiple dimensions, primarily focusing on `company_size`, `sector_proxy`, and `macro_regime`.

During the model evaluation phase, the following bias alerts were triggered and analyzed:

#### 1. Feature-Level Drift: Company Size Slices (`mega` vs. `mid` and `small`)
